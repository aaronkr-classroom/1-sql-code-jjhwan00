...
1. SQL 함수는 DBMS가 제공하는 내장 함수와 사용자가 필요에 따라 직접 정의하는 (사용자정의함)2가지로 분류한다.
2. (저장 프로시）는 미리 작성되어 DBMS에 저장된 연관된 SQL 문장들의 묶음이다.
...
5. (트랜젝）은 한 묶음으로 치리되어야 하는 데이터베이스 변경 명령어들의 집합이다.
6. 트랜잭션이 자동으로 시작되지만 반드시 사용자가 커밋이나 롤백문을 명시적으로 지정해야 커밋되는 트랜잭션 모드를 (명시）커밋 트랜잭션이라고 한다.
...
**15 다음은 트랜잭션 관련 설명으로 (Read Committed) 안의 적합한 용어를 작성하시오.**
